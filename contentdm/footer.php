    	<div class="footer">
        <p class="socialmedia">
        <a href="https://www.facebook.com/dpulibraries">
       <img src="http://libservices.org/contentdm/images/facebook.svg" />
       
       </a>
       
       <a href="https://twitter.com/intent/follow?screen_name=dpulibrarian"><img src="http://libservices.org/contentdm/images/twitter.svg" /></a>
        </p>
        <ul class="bottom-links">
            <a class="link-no-decoration bottom-links" href="mailto:dpu.webservices@gmail.com">Contact Us</a>    

            <a class="link-no-decoration bottom-links" href="http://library.depaul.edu">DePaul Library</a>

        </ul>
        <p>
        ©2011 DePaul University Library    |    Disclaimer    |    Administrative Offices, 2350 N Kenmore Avenue Chicago, IL 60614    |    (773) 325-7862
        </p>
        </div>



<!-- Google Analytics -->
<script>
  (function(i,s,o,g,r,a,m){i['GoogleAnalyticsObject']=r;i[r]=i[r]||function(){
  (i[r].q=i[r].q||[]).push(arguments)},i[r].l=1*new Date();a=s.createElement(o),
  m=s.getElementsByTagName(o)[0];a.async=1;a.src=g;m.parentNode.insertBefore(a,m)
  })(window,document,'script','//www.google-analytics.com/analytics.js','ga');

  ga('create', 'UA-5169126-10', 'depaul.edu');
  ga('send', 'pageview');

</script>
<!-- End Google Analytics -->
    

